...
from langcorrect.chats.consumers.db_operations import (
    get_groups_to_add,
    get_message_by_id,
    get_unread_count,
    get_user_by_pk,
...
from langcorrect.chats.consumers.errors import ErrorDescription, ErrorTypes
from langcorrect.chats.consumers.message_types import (
...
from langcorrect.chats.models import Message
...
class ChatConsumer(AsyncWebsocketConsumer):
    async def _after_message_save(self, msg: Message, rid: int, user_pk: str):
        ...
        await self.channel_layer.group_send(
            user_pk, OutgoingEventNewUnreadCount(sender=self.group_name, unread_count=new_unreads)._asdict()
        )
        ...
        if self.scope["user"].is_authenticated:
            ...
            for d in dialogs:  # type: int
                if str(d) != self.group_name:
                    await self.channel_layer.group_send(
                        str(d), OutgoingEventWentOnline(user_pk=str(self.user.pk))._asdict()
                    )
        else:
    ...
    async def disconnect(self, close_code):
        ...
        if close_code != UNAUTH_REJECT_CODE and getattr(self, "user", None) is not None:
            ...
            for d in dialogs:
                await self.channel_layer.group_send(
                    str(d), OutgoingEventWentOffline(user_pk=str(self.user.pk))._asdict()
                )

    async def handle_received_message(self, msg_type: MessageTypes, data: dict[str, str]) -> ErrorDescription | None:
        ...
        if (
            ...
            if msg_type == MessageTypes.IsTyping:
                ...
                for d in dialogs:
                    if str(d) != self.group_name:
                        await self.channel_layer.group_send(
                            str(d), OutgoingEventIsTyping(user_pk=str(self.user.pk))._asdict()
                        )
                return None
                ...
                for d in dialogs:
                    if str(d) != self.group_name:
                        await self.channel_layer.group_send(
                            str(d), OutgoingEventStoppedTyping(user_pk=str(self.user.pk))._asdict()
                        )
                return None
                ...
                if "user_pk" not in data:
                    ...
                    await self.channel_layer.group_send(
                        user_pk,
                        OutgoingEventMessageRead(message_id=mid, sender=user_pk, receiver=self.group_name)._asdict(),
                    )
                    recipient: AbstractBaseUser | None = await get_user_by_pk(user_pk)
                    ...
                    if not recipient:
                        ...
                        msg_res: tuple[str, str] | None = await get_message_by_id(mid)
                        if not msg_res:
                            ...
                            await self.channel_layer.group_send(
                                self.group_name,
                                OutgoingEventNewUnreadCount(sender=user_pk, unread_count=new_unreads)._asdict(),
                            )
