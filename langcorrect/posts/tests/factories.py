...
class PostFactory(DjangoModelFactory):
    ...
    text = LazyAttribute(
        lambda x: generate_text(LANGUAGE_TO_FAKER_LOCALE.get(x.language.code)),
    )
    ...
    language = LazyAttribute(
        lambda obj: random.choice(
            LanguageLevel.objects.filter(user=obj.user).exclude(level="N"),
        ).language,
    )

    gender_of_narration = LazyAttribute(
        lambda _: fake.random_element(elements=GenderChoices.choices)[0],
    )
